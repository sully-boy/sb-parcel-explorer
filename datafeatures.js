...
function fetchPermitHistory(apn, cx, cy) {
  ...
  return Promise.all([q1, q2]).then(function (results) {
    ...
    function parseFeature(f) {
      ...
      seen[id] = true;
      permits.push({
        recordId:    a.Record_ID    || a.RecordID    || a.RECORD_ID    || '—',
        recordName:  a.Record_Name  || a.RecordName  || a.RECORD_NAME  || '—',
        description: a.Description  || a.description || '—',
        status:      a.Status       || a.status      || '—',
        statusGroup: a.Status_Group || a.StatusGroup || '—',
        type:        a.Record_Type  || a.RecordType  || a.Type         || '—',
        openDate:    epochToDate(a.Open_Date   || a.OpenDate   || a.OPEN_DATE),
        bpIssuedDate: epochToDate(a.BP_Issued_Date || a.BPIssuedDate || a.BP_ISSUED_DATE),
        cofDate:     epochToDate(a.COF_Date    || a.COFDate    || a.CO_Date || a.CODate),
        aduUnits:    a.ADU_Units    || a.ADUUnits    || a.Num_ADU_Units || '—',
        address:     a.Address      || a.address     || a.SITE_ADDRESS  || '—',
        apn:         a.APN          || a.apn         || '—'
      });
    }
...
function fetchNearbyADUComps(cx, cy, apn) {
  ...
  // Build WHERE clause
  var whereClause = "Status_Group = 'C of O Issued-Project Complete'";
  if (apnClean) {
  ...
  return dfFetch(264, {
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      comps.push({
        address:  a.Address      || a.address      || a.SITE_ADDRESS  || '—',
        aduUnits: a.ADU_Units    || a.ADUUnits      || a.Num_ADU_Units || '—',
        cofDate:  epochToDate(a.COF_Date  || a.COFDate  || a.CO_Date),
        type:     a.Record_Type  || a.RecordType    || a.Type          || '—',
        description: a.Description || a.description || '—'
      });
    }
...
function analyzeOwnership(parcelAttrs) {
  ...
  var owner = String(p.Owner || p.OWNER || p.owner || '').toUpperCase().trim();
  var landValue = parseInt(p.LandValue || p.LAND_VALUE || 0, 10) || 0;
  var netAV = parseInt(p.Net_AV || p.NET_AV || p.NETAV || 0, 10) || 0;
  var yearBuilt = parseInt(p.YearBuilt || p.YEAR_BUILT || 0, 10) || 0;
