...
async function loadParcelsInView() {
  ...
  try {
    ...
    // Keep parcels above zoning layer for click handling
    if (state.activeFeatureLayers['parcels']) {
      state.activeFeatureLayers['parcels'].bringToFront();
    }
  } catch (e) {
    setLoading(false);
...
async function loadZoningInView() {
  ...
  try {
    ...
    addOrReplaceLayer('zoning', geojson, (feature) => {
      const zone = feature.properties.Zone_1 || feature.properties.ZONE || '';
      ...
      const p = feature.properties;
      const zone = p.Zone_1 || p.ZONE || '—';
      const label = ZONE_LABELS[zone] || zone;
      layer.bindTooltip(`<b>${zone}</b><br/>${label}`, { sticky: true, className: 'zone-tooltip' });
    });
    // Push zoning below parcels so parcels always receive clicks
    if (state.activeFeatureLayers['zoning']) {
      state.activeFeatureLayers['zoning'].bringToBack();
    }
...
async function loadFemaFlood() {
  try {
    ...
    const sfhaFilter = "FLD_ZONE NOT IN ('X', 'X PROTECTED BY LEVEE', 'AREA NOT INCLUDED', 'OPEN WATER')";
    ...
    addOrReplaceLayer('fema-flood', geojson, {
      ...
      const p = feature.properties;
      layer.bindTooltip(`FEMA Flood Zone: ${p.FLD_ZONE || p.ZONE_DESC || '—'}`, { sticky: true });
    });
...
function selectParcel(feature, leafletLayer) {
  ...
  highlightTableRow(feature.properties?.OBJECTID || feature.properties?.APN);
...
async function loadAdjacentData(parcelFeature) {
  ...
  const [zoningRes, chipsRes, hfRes, ffRes, coastRes, histRes, tpaRes, transitHMRes, gpRes, priorityHsgRes] = await Promise.allSettled([
    queryLayer(CONFIG.LAYERS.zoning, { ...pointParams, outFields: 'Zone_1,Zone_Descr,ZoneOther,ZONEDESG,OverlayZone,S_D_3' }),
    ...
    Promise.resolve({ features: [] }), // layer 294 excluded — half-mile bus buffer is too broad for AB 2097
    queryLayer(CONFIG.LAYERS.generalPlan, { ...pointParams, outFields: 'LUCode,Land_Use_Designations,GPorLCP' }).catch(() => ({ features: [] })),
  ...
  // AB 2097 eligibility uses layer 284 (TPA) as sole source.
  // Layer 294 (half-mile bus buffer) covers most of the city — excluded to avoid false positives.
  ...
  const gpDesignation = gpAttrs?.Land_Use_Designations || gpAttrs?.LUCode || null;
  let audTier = null;
  if (gpDesignation) {
  ...
  const zoneStr = zoningAttrs?.Zone_1 || zoningAttrs?.ZONEDESG || '';
  ...
  const context = {
    ...
    floodZoneType:        floodAttrs?.FLD_ZONE || floodAttrs?.ZONE_DESC || null,
  ...
  const _apn  = parcelAttrs.APN || parcelAttrs.Apn9 || '';
...
function renderDeveloperReport(r) {
  ...
  const lawColors = {
    ...
    'AR':           '#0f766e', // teal
    'AB2011':       '#b45309', // amber
    'SB35':         '#6b7280', // grey — not applicable
  ...
  (mu.breakdown || []).forEach(function(b) {
    ...
    if (shortLabel === 'ADU') {
      ...
      // AR has no unit count — show a building icon with "Eligible" instead
      cards.push(
...
function makeLawCard(law, subtitle, analysis, type) {
  ...
  if (type === 'sb9' && isEligible) {
    ...
    if (analysis.incentives && analysis.incentives.length) {
      ...
      analysis.incentives.forEach(function(i) {
        details += '<div class="law-pathway-note" style="padding-left:8px">✓ ' + escHtml(i) + '</div>';
      });
    }
    ...
    if (analysis.notes && analysis.notes.length) {
      analysis.notes.forEach(function(n) {
        details += '<div class="law-warning" style="margin-top:4px">' + escHtml(n) + '</div>';
      });
    }
    ...
    if (!analysis.hasCorridor) {
      details += '<div class="law-note" style="margin-top:6px">Pathway B (Mixed-Income Corridor) not available — parcel is not within ½ mile of a major transit stop. Only Pathway A (100% Affordable) applies.</div>';
    }
    details +=
    ...
    // SB 35 always shown as not applicable for City of SB — grey informational card
    details =
...
function showParcelDetail(feature) {
  ...
  const apn  = p.APN || p.Apn9 || '—';
  const addr = [p.Situs1, p.Situs2].filter(Boolean).join(' ') || [p.SNum, p.SDir, p.SStreet, p.SStreetSuf].filter(Boolean).join(' ') || '—';
...
function populateParcelTab(p) {
  ...
  grid.innerHTML = [
    makeRow('APN', `<span class="highlight">${fmtVal(p.APN || p.Apn9)}</span>`),
    makeRow('Address', fmtVal(addr)),
    makeRow('Owner', fmtVal(p.Owner || p.ConOwner)),
    makeRow('Land Use', fmtVal(p.LandUse)),
    makeRow('Use Code', fmtVal(p.UseCode)),
    makeRow('Acreage', fmtAcre(p.Acreage)),
    makeRow('Sq Footage', fmtSqft(p.SqFootage)),
    ...
    makeRow('Net AV', `<span class="money">${fmtCurrency(p.Net_AV)}</span>`),
...
function populateZoningTab(zp) {
  ...
  const zone = zp.Zone_1 || zp.ZONE || '—';
  const zoneColor = ZONE_COLORS[zone] || '#cccccc';
  const label = ZONE_LABELS[zone] || zp.Zone_Descr || '—';
  grid.innerHTML = [
...
function populateFireTab(chipData, isHighFire, floodData) {
  ...
  grid.innerHTML = [
    ...
    '<div style="height:4px;border-bottom:1px solid var(--color-divider);margin:4px 0"></div>',
    makeRow('FEMA Flood Zone', floodData ? fmtVal(floodData.FLD_ZONE || floodData.ZONE_DESC || 'Yes') : 'Not in flood zone'),
...
function populateResultsTable(features) {
  ...
  tbody.innerHTML = features.slice(0, CONFIG.MAX_RESULTS).map(f => {
    ...
    return `<tr data-objectid="${p.OBJECTID}" data-apn="${p.APN || ''}">
      ...
      <td title="${p.Owner || ''}">${p.Owner || '—'}</td>
      <td title="${p.LandUse || ''}">${p.LandUse || '—'}</td>
      <td>${p.Acreage ? Number(p.Acreage).toFixed(3) : '—'}</td>
      <td style="font-family:var(--font-mono)">${p.Net_AV ? '$' + Number(p.Net_AV).toLocaleString() : '—'}</td>
    </tr>`;
  }).join('');
  ...
  try {
    ...
    if (isAPN(q)) {
      ...
      const data = await queryLayer(CONFIG.LAYERS.parcels, {
        ...
        outFields: 'APN,Situs1,Situs2,Owner,LandUse',
      ...
      results = (data.features || []).map(f => ({
        ...
        sub: [f.attributes.Situs1, f.attributes.Situs2].filter(Boolean).join(' ') || f.attributes.Owner || '—',
      ...
      results = (data.features || []).map(f => ({
        ...
        sub: `APN: ${f.attributes.APN || '—'} · ${f.attributes.Owner || ''}`,
        apn: f.attributes.APN,
      }));
    }
