...
(function () {
  ...
  function updateParcelHeader(apn, address, lat, lng, report, pfResult) {
    ...
    const compareClass = inCompare ? 'compare-btn in-list' : 'compare-btn';
    const compareLabel = inCompare ? '− Compare' : '+ Compare';
    const compareBtn   = `
    ...
    document.getElementById('compareToggleBtn').addEventListener('click', () => {
      const nowInList = CompareBar.toggleSlot({ apn, address });
      // Also sync full parcel data into window.Comparison so renderTable works
      if (window.Comparison) {
        if (nowInList) {
          // Feed full currentParcelData if available, otherwise minimal stub
          ...
          window.Comparison.removeParcel(apn);
        }
      }
      updateParcelHeader(apn, address, lat, lng, report, pfResult); // re-render
    });
