...
function analyzeParcel(parcelAttrs, zoningAttrs, context) {
  ...
  const zone = z.Zone_1 || z.ZONE || '';
  const zoneOther = z.ZoneOther || z.ZONE_OTHER || zone;
  const zoneDesc = z.Zone_Descr || '';
  ...
  const assessorSqft = parseFloat(p.SqFootage) || 0;
  ...
  const netAV = parseInt(p.Net_AV) || 0;

  const category = getZoneCategory(zone);
  const isSF = isZone(zone, 'SINGLE_FAMILY');
  const isMF = isZone(zone, 'MULTI_FAMILY');
  const isComm = isZone(zone, 'COMMERCIAL');
  ...
  const baseRights = getBaseZoningRights(zone, lotSqft, acreage, p, isSF, isMF, isComm);

  // ── 2. ADU Analysis (State law applies everywhere) ─────────
  const aduAnalysis = analyzeADU(zone, lotSqft, isSF, isMF, isCoastal, isHighFire, nearTransit, p, existingUnits, audTier);

  // ── 3. SB 9 Analysis ──────────────────────────────────────
  const sb9Analysis = analyzeSB9(zone, lotSqft, isSF, isCoastal, isHighFire, isHistoric, isFlood, p, ctx);

  // ── 4. SB 684 / SB 1123 (Starter Home) ───────────────────
  const shraAnalysis = analyzeSHRA(zone, lotSqft, isSF, isMF, isCoastal, isHighFire, isHistoric, isFlood, acreage, p, audTier);

  // ── 5. AB 2097 Parking ────────────────────────────────────
  const parkingAnalysis = analyzeAB2097(nearTransit, inTPA, zone);

  // ── 6. Coastal restrictions ───────────────────────────────
  const coastalAnalysis = analyzeCoastal(isCoastal, ctx, zone);

  // ── 7. Fire Restrictions ──────────────────────────────────
  const fireAnalysis = analyzeFire(isHighFire, chipZone, zone);
  ...
  // ── 10. Buildable area estimate ───────────────────────────
  const buildableCalc = calcBuildableArea(zone, lotSqft, acreage, p);

  // ── Max unit count summary ────────────────────────────────
  const dblAnalysis   = analyzeDBL(zone, isMF, isCoastal, audTier, lotSqft);
  const arAnalysis    = analyzeAR(zone, isCoastal, isHistoric, yearBuilt);
  const ab2011Analysis = analyzeAB2011(zone, lotSqft, isCoastal, isHistoric, isFlood, isHighFire, nearTransit);
  const sb35Analysis   = analyzeSB35(isCoastal);
  const maxUnits = calcMaxUnits(zone, sb9Analysis, aduAnalysis, shraAnalysis, dblAnalysis, arAnalysis, ab2011Analysis, baseRights, lotSqft, audTier, isCoastal, isHighFire);

  // ── Permits likely required ───────────────────────────────
  const permitsRequired = buildPermitList(isCoastal, isHighFire, isHistoric, zone, sb9Analysis, shraAnalysis, ctx);

  // ── Overall feasibility score ─────────────────────────────
  const score = calcFeasibilityScore(isSF, isMF, isComm, isCoastal, isHighFire, isHistoric, isFlood, sb9Analysis, shraAnalysis, lotSqft, zone);

  // ── Extra regulatory flags ──────────────────────────────────
  const inclusionaryFlag  = analyzeInclusionary(maxUnits, zone, isMF);
  const prevailingWageFlag= analyzePrevailingWage(zone, sb9Analysis, shraAnalysis, ctx, maxUnits);
  const oddsFlag          = analyzeODDS(zone, isMF, isCoastal, isHistoric);
...
function calcBaseDensity(zone, lotSqft, audTier, isCoastal) {
  ...
  return {
    ...
    note: `AUD: ${lotSqft.toLocaleString()} SF × ${duAc} du/ac ÷ 43,560 = ${audUnits} units`,
    audTier: audTier,
...
function calcBuildableArea(zone, lotSqft, acreage, p) {
  ...
  const existingSqft = parseInt(p.SqFootage) || 0;
...
function calcMaxUnits(zone, sb9, adu, shra, dbl, ar, ab2011, base, lotSqft, audTier, isCoastal, isHighFire) {
  ...
  // AR — sentinel entry with no unit count; grid card renderer handles specially
  if (ar && ar.eligible) {
    breakdown.push({
      law: 'Adaptive Reuse',
      units: null,
      note: 'Eligible — unit count depends on existing building',
    });
  }

  // AB 2011 — Pathway B (corridor, near transit) has a calculable unit count
  // Pathway A (100% affordable) uses local zoning density — no separate entry needed
  if (ab2011 && ab2011.eligible && ab2011.hasCorridor && ab2011.corridorUnits > 0) {
    breakdown.push({
      law: 'AB 2011 (Mixed-Income)',
      units: ab2011.corridorUnits,
      note: `${ab2011.corridorUnits} units via mixed-income corridor pathway (1 unit/545 sf); 15% VLI or 24% low income required; prevailing wage required`,
...
// Effective November 20, 2025. Applies to existing nonresidential buildings
// converted to residential in qualifying zones. No unit count — depends on
...
function analyzeAR(zone, isCoastal, isHistoric, yearBuilt) {
  ...
  if (!eligible) {
    return {
      eligible: false,
      reason: `Adaptive Reuse applies to zones that allow multi-unit residential (R-M, R-MH, O-R, O-M, C-R, C-G, M-C) — ${zone} does not qualify.`,
      citation: 'SBMC § 30.185.045 (Ord. 6191, eff. Nov. 20, 2025)',
    };
  }

  // Building age check — certificate of occupancy must be ≥5 years old.
  // yearBuilt is approximate; flag if recent.
  const currentYear = 2026;
  const ageWarning = (yearBuilt && (currentYear - parseInt(yearBuilt)) < 5)
    ? `Building year (${yearBuilt}) may not meet the 5-year certificate of occupancy requirement — verify with City.`
    : null;

  // State Street ground floor retention flag
  const onStateStreet = false; // not detectable from parcel data — surfaced as informational note

  const notes = [];
  if (isHistoric) notes.push('Designated Historic Resource — must comply with SBMC Ch. 30.157 (Historic Resources) during conversion.');
  ...
  return {
    ...
    unitNote: 'Unit count depends on existing building square footage — no theoretical maximum calculable from parcel data alone.',
    ...
    approval: isCoastal
      ? 'Coastal Zone — Coastal Development Permit required'
      : 'Exempt from discretionary review — building permit only (exterior additions or subdivision may trigger design review)',
    bapNote: 'Unit count is entirely dependent on existing building size and layout. Contact Bildsten Architecture & Planning to evaluate conversion potential for a specific building.',
    citation: 'SBMC § 30.185.045 (Ord. 6191, eff. Nov. 20, 2025)',
  };
}

// ── AB 2011 — Affordable Housing and High Road Jobs Act ───────
// Gov. Code § 65912.100 et seq. Eff. July 1, 2023.
// Two pathways:
//   A) 100% Affordable: any commercial zone, ministerial, density = local zoning
//   B) Mixed-Income Corridor: commercial corridor zone + within ½ mi of major transit,
//      1 unit/545 sf (up to 80 du/ac near transit), prevailing wage required.
// Applies regardless of RHNA status — no housing element dependency.
const AB2011_ELIGIBLE_ZONES = new Set([
  'C-G','C-R','C-1','C-2','C-P','O-R','O-M','ACS',  // inland commercial
  'C-L','C-M','OC',                                   // coastal commercial
]);
...
function analyzeAB2011(zone, lotSqft, isCoastal, isHistoric, isFlood, isHighFire, nearTransit) {
  ...
  if (!eligible) {
    return {
      eligible: false,
      reason: `AB 2011 applies to commercial zones where office, retail, or parking are principally permitted — ${zone} does not qualify.`,
      citation: 'Gov. Code § 65912.100 et seq. (AB 2011, eff. July 1, 2023)',
    };
  }

  // Hard site exclusions per § 65912.111(a)
  if (isHistoric) {
    return {
      eligible: false,
      reason: 'Historic structure on parcel — AB 2011 prohibits demolition of individually listed historic buildings.',
      citation: 'Gov. Code § 65912.111(a)',
    };
  }
  if (isFlood) {
    return {
      eligible: false,
      reason: 'FEMA Special Flood Hazard Area — AB 2011 site criteria exclude special flood hazard areas.',
      citation: 'Gov. Code § 65912.111(a)',
    };
  }
  if (isHighFire) {
    return {
      eligible: false,
      reason: 'High/Very High Fire Hazard Severity Zone — AB 2011 site criteria exclude HFHSZ parcels.',
      citation: 'Gov. Code § 65912.111(a)',
    };
  }

  // Pathway B: Mixed-Income Corridor — requires ½ mile of major transit stop
  const hasCorridor = nearTransit;

  // Density calculation for Pathway B (near transit)
  // 1 unit per 545 sf of lot area, capped at 80 du/ac
  ...
  return {
    ...
    pathways: [
      {
        label: 'Pathway A — 100% Affordable',
        description: 'All units deed-restricted affordable. Density per local zoning (AUD where applicable). Ministerial, no CEQA.',
        affordability: '100% affordable (income mix per project)',
        approval: isCoastal ? 'Coastal Zone — CDP required' : 'Ministerial — no CEQA, no discretionary review',
        prevailingWage: 'Required',
      },
      ...(hasCorridor ? [{
        label: 'Pathway B — Mixed-Income Corridor',
        description: `Up to ${corridorUnits} units (1 unit/545 sf, max 80 du/ac). Min. 15% very low income or 24% low income.`,
        affordability: '15% very low income OR 24% low income (deed-restricted)',
        units: corridorUnits,
        approval: isCoastal ? 'Coastal Zone — CDP required' : 'Ministerial — no CEQA, no discretionary review',
        prevailingWage: 'Required',
        heightBonus: '65 ft max (if within ½ mi transit and AB 2097 applies)',
        corridorDisclaimer: 'Pathway B eligibility shown because this parcel is within ½ mile of a major transit stop, which approximates Santa Barbara\'s qualifying commercial corridors. AB 2011 formally requires commercial zoning on both sides of the street for ½ mile — verify corridor qualification with City Planning before relying on this pathway.',
      }] : []),
    ],
    prevailingWageNote: 'All AB 2011 projects require prevailing wage and, for 50+ unit projects, apprenticeship program participation.',
...
// Gov. Code § 65913.4. Applies only to jurisdictions NOT meeting RHNA targets.
// As of February 13, 2024, the City of Santa Barbara has a compliant Housing Element
// and adequate sites to meet 6th Cycle RHNA (HCD finding). SB 35 therefore does NOT
// apply to City of Santa Barbara parcels. Shown as informational card only.
...
function analyzeRentFreeze(yearBuilt, p) {
  ...
  const hasExistingResidential = !!(p && (p.SqFootage > 0 || p.Bedrooms > 0));
